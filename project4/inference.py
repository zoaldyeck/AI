...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    #print "E1(noisy):",noisyDistance
    #print "P(Et+1|Xt+1):",emissionModel
    ...
    if noisyDistance == None: allPossible[self.getJailPosition()] = 1.0
    else:
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            #print "ghost can be in ",p
            #print "if ghost is in p, the trueDistance is ",trueDistance
            #print "P(e1|X1=true)=",emissionModel[trueDistance]
  ...
  def elapseTime(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    "P(Xt+1|e1:t)=sigmaP(Xt+1|Xt)P(Xt|e1:t)"
    allPossible = util.Counter()
    for p in self.legalPositions:
        #print "ghost can be :",p
        newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
        #print "ghost then can be in ",newPosDist
        for newPos,prob in newPosDist.items():
            allPossible[newPos] += prob * self.beliefs[p]
            #print newPos," possibility is ",prob,"*",self.beliefs[p],"=",allPossible[newPos]
    allPossible.normalize()
    self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    self.particlesList = []
    ghostPostion = self.legalPositions
    i = self.numParticles
    while i > 0:
        for ghostpos in ghostPostion:
            if i <= 0:
                ...
                self.particlesList.append(ghostpos)
                i = i - 1
    #print "Initially,",len(self.particlesList)
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance == None: 
        ...
        #print "particlesList is ",self.particlesList
        for pos in self.particlesList:
            #print "pos is ",pos
            disForPos = util.manhattanDistance(pos,pacmanPosition)
            #print "if in this, the distance should be ",disForPos
            temp[pos] += emissionModel[disForPos]
            #print "the noisy evidence gives the probability of the true distance is ",emissionModel[disForPos]
            #print "checking the list:",pos,"the weight = ",temp[pos]
            #print yy
        if temp.totalCount() == 0:
            ...
            self.particlesList = []
            #print "temp is",temp
            #print "after sample:",util.sample(temp)
            #print yy
            for i in range(self.numParticles):
                self.particlesList.append(util.sample(temp))
...
class MarginalInference(InferenceModule):
  ...
  def getBeliefDistribution(self):
    ...
    #print "jointDistribution is ",jointDistribution
    #print "the len of ",len(jointDistribution)
    dist = util.Counter()
    for t, prob in jointDistribution.items():
      #print "t is ",t,"self.index-1=",self.index-1
      #print "t[self.index-1]=",t[self.index-1]
      dist[t[self.index - 1]] += prob
    #print "in where it's wrong the dist is ",dist
    return dist
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    "*** YOUR CODE HERE ***"
    self.particles = list()
    ghostItem = self.numGhosts
    single = []
    totalPositions=[]
    totalPositions=[self.unsortedList()for i in range(ghostItem)]
    for i in range(self.numParticles):
        single = []
        for ghost in range(ghostItem):
            single.append(totalPositions[ghost][i])
        self.particles.append(tuple(single))
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
        ...
        #print "newParticle is ",newParticle
        #print yy
        "*** YOUR CODE HERE ***"
        for i in range(self.numGhosts):
            newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle),i, self.ghostAgents[i])
            #print 'newPosDist is the elapse time result',newPosDist
            newParticle[i] = util.sample(newPosDist)
        #print "after sample, ",i,"ghost is ",newParticle[i]
        newParticles.append(tuple(newParticle))
        #print "then the newParticles is ",newParticles
