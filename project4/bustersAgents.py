...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    #print "pacman is in ",pacmanPosition
    #print "pacman can do ",legal
    #print "Ghosts",livingGhosts
    livingGhostPositionDistributions = [beliefs for i,beliefs
                                        in enumerate(self.ghostBeliefs)
                                        if livingGhosts[i+1]]
    #print "ghost XX possible position is ",livingGhostPositionDistributions
    "*** YOUR CODE HERE ***"
    priorQueue = util.PriorityQueue()
    for action in legal:
        mazeDistance = 0
        successorPosition = Actions.getSuccessor(pacmanPosition, action)
        #print "successorPostion=",successorPosition
        for ghostI in livingGhostPositionDistributions:
            #print "single ghost:",ghostI
            for ghostPos,prob in ghostI.items():
                #print "ghostPos, prob",ghostPos,prob
                if prob != 0:
                    mazeDistance += self.distancer.getDistance(successorPosition, ghostPos)*prob
                    #print "mazeDistance = ",mazeDistance
            priorQueue.push(action,mazeDistance)
    bestAction = priorQueue.pop()            
    return bestAction           
