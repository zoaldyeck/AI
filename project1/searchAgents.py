...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            x,y = state[0]
            corners=state[1]
            leftcorners=list(corners)
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            hitsWall = self.walls[nextx][nexty]
            if not hitsWall:
                nextState = (nextx, nexty)
                if nextState in leftcorners:
                    leftcorners.remove(nextState)
                successors.append(((nextState,leftcorners),action, 1))
        self._expanded += 1
        return successors
...
def cornersHeuristic(state, problem):
    ...
    heuristic=0
    while len(leftcorners):
        temp=9999999999
        for corner in leftcorners:
            if temp>util.manhattanDistance(current,corner):
                temp=util.manhattanDistance(current,corner)
                nextcorner=corner
        heuristic=heuristic+temp
        current=nextcorner
        leftcorners.remove(nextcorner)
    return heuristic
    ...
    while not stack.isEmpty():
        father=stack.pop()
        currentsum=father[1]
        currentpositon=father[0]
        for corner in leftcorners:
            nextsum=util.manhattanDistance(currentpositon,corner)
            if len(leftcorners)==0:
                visitied.append(corner)
                sum=currentsum+nextsum
                return sum
            if not corner in visitied:
                visitied.append(corner)
                sum=currentsum+nextsum
                stack.push((corner,sum))
    return sum #ault to trivial solution
    """
...
def foodHeuristic(state, problem):
    ...
    wall=problem.walls.asList()
    heuristic=0
    while len(foodPosition):
        temp=9999999999999
        for food in foodPosition:
            if temp>util.manhattanDistance(position,food):
                temp=util.manhattanDistance(position,food)
                nextfood=food
        heuristic=heuristic+temp
        position=nextfood
        foodPosition.remove(nextfood)
    return heuristic
"""
    position, foodGrid = state
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        if len(lfood):
            temp=99999999
            for foodpoint in lfood:
                if temp>util.manhattanDistance(startPosition,foodpoint):
                    temp=util.manhattanDistance(startPosition,foodpoint)
                    nextpoint=foodpoint
        
        queue.push((startPosition,[]))
        visited.append(startPosition)    
                #put all the foodpoint into the priorityqueue
        while not queue.isEmpty():    #for the nearest foodpoint
            father=queue.pop()        # choice pop out
            path=father[1]
            if father[0]==nextpoint:
                lfood.remove(nextpoint)
                return path
            for cord,action,cost in problem.getSuccessors(father[0]):
                if cord not in visited:
                    visited.append(cord)
                    path=father[1]+[action]
                    queue.push((cord,path))
        util.raiseNotDefined()
